...
def main():
    parent = "scope:source.shell.{}#".format(os.path.splitext(sys.argv[1])[0].split("-")[-1])
    ...
    for command, value in commands_input.items():
        ...
        cmd_args_bt.append({ "meta_scope": "meta.function-call.arguments.shell" })
        ...
        main.append({
            "match": match,
            "scope": scope,
            "set": ["{}cmd-post".format(parent), "cmd-args-{}".format(command)]
        })
        main_bt.append({
            "match": match,
            "scope": scope,
            "set": ["{}cmd-post-bt".format(parent), "cmd-args-{}-bt".format(command)]
        })
    contexts["prototype"] = [{
